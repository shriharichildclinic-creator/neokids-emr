/**
 * PDF Generation: Invoices + Prescriptions (pdfkit)
 */
...
function drawHeader(doc, title) {
  ...
  doc.fillColor('white').fontSize(22).font('Helvetica-Bold')
     .text('NeoKidsPro', 50, 28);
  doc.fontSize(10).font('Helvetica')
     .text('Pediatric Clinic · neokidspro.in', 50, 55);
...
async function generatePrescription(appointment, prescription) {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.font('Helvetica-Bold').text('Phone: ', 50, py + 15, { continued: true })
    ...
    doc.moveTo(50, py + 40).lineTo(doc.page.width - 50, py + 40).strokeColor(BRAND_BLUE).stroke();
    ...
    if (prescription.allergies) section('Allergies', prescription.allergies);
    if (prescription.investigations) section('Investigations', prescription.investigations);
    ...
    if (prescription.advice) section('Advice', prescription.advice);
    ...
    doc.fontSize(10).fillColor('#555')
       .text('___________________________', doc.page.width - 220, sigY);
    doc.font('Helvetica-Bold').fillColor('#000')
       .text(`Dr. ${appointment.doctor.name}`, doc.page.width - 220, sigY + 12);
    doc.font('Helvetica').fillColor('#555').fontSize(9)
       .text('Digital Signature', doc.page.width - 220, sigY + 26);
