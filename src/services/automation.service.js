...
// Claim-guard for AUTOMATIC (issue-time) document delivery. The first auto
// trigger for a given document+channel inserts a unique claim row; any
// duplicate trigger (double-fired hook, retried job, two tabs issuing at
// once) gets P2002 and is suppressed instead of messaging the patient twice.
// Manual sends never set auto=true, so an explicit resend always goes through.
async function claimAutoDelivery(claimKey) {
  try {
    await prisma.notificationLog.create({
      data: { claimKey, appointmentId: null, channel: 'WHATSAPP', recipient: '-', template: '__auto_delivery_claim__', direction: 'PATIENT', status: 'SENT' }
    });
    return true;
  } catch (e) {
...
// ─────────────────────────────────────────────────────────────────────
async function onCertificateIssued({ certificate, pdfRes, sendWhatsapp = true, sendEmail = true, auto = false }) {
  ...
  // Auto (issue-time) sends are claim-guarded per channel so a duplicate
  // auto trigger can never deliver the same certificate twice; a manual
  // resend (auto=false) skips the claim and always sends.
  const autoEmailOk    = !auto || (await claimAutoDelivery(`cert_auto_email_${certificate.id}`));
  const autoWhatsappOk = !auto || (await claimAutoDelivery(`cert_auto_wa_${certificate.id}`));

  if (sendEmail && autoEmailOk) {
    if (patient.email) {
      await safeEmail({
  ...
  if (sendWhatsapp && autoWhatsappOk && patient.phone) {
...
// Regenerate and (re)deliver the consultation invoice for an online-PAID
// IN-CLINIC (OFFLINE) booking — the case where the automated confirmation
// flow confirmed payment (cashfreeOrderId set) but its PDF/send step failed,
// leaving invoiceUrl null and the patient without an invoice. Generates the
// PDF if missing, persists invoiceUrl, then sends over WhatsApp + email.
// Deliberately separate from onOnlineBookingConfirmed: that flow is for
// teleconsultations and creates a Google Meet link, which an in-clinic visit
// must never do.
async function deliverOfflineInvoice(appointment) {
  const a = appointment;
  const apptShort = a.id.slice(0, 8).toUpperCase();
  const filename = `invoice_${apptShort}.pdf`;
  let invoiceUrl = a.invoiceUrl || null;
  let invoiceFilepath = null;
  try {
    const inv = await pdf.generateInvoice(a);
    ...
    logger.error('deliverOfflineInvoice: PDF generation failed', e);
    return { whatsapp: 'pdf_failed', email: 'pdf_failed', invoiceUrl };
  }
  if (!a.invoiceUrl && invoiceUrl) {
    await prisma.appointment.update({ where: { id: a.id }, data: { invoiceUrl } }).catch(() => {});
  }
  const delivery = { whatsapp: 'skipped', email: 'skipped' };

  if (a.patient && a.patient.phone) {
    try {
      const r = await waMedia.sendInvoicePdf({ appointment: a, filepath: invoiceFilepath, publicUrl: invoiceUrl });
      await logNotification({
        appointmentId: a.id, channel: 'WHATSAPP', recipient: a.patient.phone,
        template: process.env.WA_TPL_INVOICE_PDF || 'neokids_invoice_pdf',
        direction: 'PATIENT', status: 'SENT', payload: r || undefined
  ...
  if (a.patient && a.patient.email) {
    await safeEmail({
      appointmentId: a.id, recipient: a.patient.email, template: 'IN_CLINIC_INVOICE', direction: 'PATIENT',
      messageFactory: () => email.sendEmail({
        to: a.patient.email,
        subject: `Your consultation invoice - NeoKidsPro`,
        html: renderBrandedEmail({
          preheader: `Your consultation invoice for Dr. ${a.doctor.name} is attached.`,
          headline: 'Consultation Invoice',
          subhead: `Dr. ${esc(a.doctor.name)}`,
          bodyHtml: `<p>Dear ${esc(a.patient.name)},</p><p>Thank you for visiting. Your consultation invoice is attached to this email as a PDF.</p>`,
          footerNote: `Regards,<br>${esc(getClinicNameForBody(a.doctor))}`
        }),
        attachments: [{ filename, path: invoiceFilepath }]
      })
    });
    delivery.email = 'sent';
  }

  return { ...delivery, invoiceUrl };
}
